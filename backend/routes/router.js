...
router.get("/payment/callback", async (req, res) => {
  try {
    const {
      ...
      paySuccessTime,
      sign,
      param1,   // ← contains userId (we set this when creating order)
      param2,
      utr,
    } = req.query;
    ...
    if (sign !== expectedSign) {
      ...
      // Still return success to avoid gateway retrying with bad sign
      return res.status(200).send("success");
    }

    // ── Find the deposit in our DB ────────────────────────────────────────
    const deposit = await Deposit.findOne({ mchOrderNo });

    if (!deposit) {
      console.error("⚠️  [callback] deposit not found for mchOrderNo:", mchOrderNo);
      return res.status(200).send("success"); // Must return success to stop retries
    }

    // ── Prevent double-processing (idempotency check) ─────────────────────
    if (deposit.status === "completed") {
      console.log("ℹ️  [callback] already completed, skipping:", mchOrderNo);
      return res.status(200).send("success");
    }
    ...
    switch (paymentStatus) {
      ...
      case 1: {
        // ✅ PAYMENT SUCCESS
        console.log(`✅ Payment SUCCESS — Order: ${mchOrderNo} | Amount: ${realAmount} | UTR: ${utr || "N/A"}`);
        ...
        await Deposit.findOneAndUpdate(
          { mchOrderNo },
          {
            ...
            utr: utr || null,
            paySuccessTime: new Date(Number(paySuccessTime)),
          }
        );

        // Credit user balance
        // param1 contains the userId we stored during order creation
        if (param1) {
          const creditAmount = Number(realAmount); // in cents — adjust if you store in rupees/taka
          ...
          console.log(`💰 Credited user ${param1} with ${creditAmount} cents`);
        }
      ...
      case 3: {
        // ⏰ TIMEOUT (may still complete later, gateway won't callback again for timeout)
        console.warn(`⏳ Payment TIMEOUT — Order: ${mchOrderNo}`);

        await Deposit.findOneAndUpdate(
          { mchOrderNo },
          { status: "timeout", payOrderId }
        );
      ...
      case 10:
      default: {
        // ℹ️ Still pending/processing — just log
        console.log(`ℹ️  Payment status ${paymentStatus} — Order: ${mchOrderNo}`);
        break;
      }
    }

    // ── MUST return "success" — stops gateway retry loop ─────────────────
    return res.status(200).send("success");
